...
```bash
# Check status
docker exec -it clawdbot_gateway_1 node dist/index.js status

# Run onboarding wizard
docker exec -it clawdbot_gateway_1 node dist/index.js onboard

# Login to WhatsApp (scan QR)
docker exec -it clawdbot_gateway_1 node dist/index.js channels login

# Add Telegram bot
docker exec -it clawdbot_gateway_1 node dist/index.js channels add --channel telegram --token "YOUR_TOKEN"

# Check health
docker exec -it clawdbot_gateway_1 node dist/index.js health

# Run doctor
docker exec -it clawdbot_gateway_1 node dist/index.js doctor
```
...
1. SSH into Umbrel and exec into the container:
   ```bash
   docker exec -it clawdbot_gateway_1 bash
   ```
...
**Reset configuration:**
```bash
docker exec -it clawdbot_gateway_1 node dist/index.js reset --config
```
...
docker exec -it clawdbot_gateway_1 ps aux | grep Xvfb
```
